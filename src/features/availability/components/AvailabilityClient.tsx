...
import { checkAvailabilityToServices, createAvailabilityAction, updateAvailabilityAction, deleteAvailabilityAction } from '../server/actions'
...
export default function AvailabilityClient({ availabilitiesData, defaultAvailabilityData }: PageProps) {
    ...
    const uploadAvailability = async (isEdit: boolean, index?: number) => {
        if (!isEdit) {
            const result = await createAvailabilityAction(user.business_id!, availability)
            setAvailabilities([...availabilities, result])
            setConfirmationData({ title: "Success", description: "Availability Created" })
            ...
            if (index != undefined) {
                const result = await updateAvailabilityAction(
                    ...
                    availability,
                    availability.id,
                    isDefault ? availability.id : defaultAvailable
                )
            ...
            setisEditing(false)
            setConfirmationData({ title: "Success", description: "Availability Updated" })
            setConfirmation(true)
        }
    ...
    const handleDelete = async (index: number) => {
        ...
        if (hasAttachedServices instanceof PostgrestError) {
            ...
            await deleteAvailabilityAction(user.business_id!, availability.id)
            let clone = [...availabilities]
            clone.splice(index, 1)
            setAvailabilities(clone)
            setConfirmationData({ title: "Success", description: "Availability Deleted" })
            setConfirmation(true)
        }
    }
