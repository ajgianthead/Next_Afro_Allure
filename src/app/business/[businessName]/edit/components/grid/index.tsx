...
export const GridLayoutComponent: any = {
    ...
    inline: true,
    render: (({ gapY, justifyItems, gapX, alignItems, cells, puck, numberOfColumns, numberOfRows, firstCellRowSpan, firstCellColumnSpan, mobileColumns = 1, opacity }: {
        gapX: number, numberOfRows: number, gapY: number, numberOfColumns: number, mobileColumns: number, cells: { cell: SlotComponent }[], alignItems: string, justifyItems: string, puck: any, firstCellRowSpan: number, firstCellColumnSpan: number, opacity?: number
    }) => {
        ...
        return (
            <div className={`grid w-full ${colClass}`} style={{ gridTemplateRows: `repeat(${numberOfRows}, minmax(0, 1fr))`, columnGap: gapX, rowGap: gapY, alignItems, justifyItems, opacity: opacity != null ? opacity / 100 : undefined }} ref={puck.dragRef}>
