...
import { Box, Columns2, Loader2, Palette, Type } from "lucide-react";
...
// Render Puck editor
function Editor({ businessId, editorData, businessName, services, isPublished, initialTheme }: PageProps) {
    ...
    return <Puck config={config} data={editorData.length > 0 ? JSON.parse(editorData) : undefined} onAction={(action, state) => {
            ...
            return (
                <>
                    ...
                    <Button
                        ...
                        style={{ borderColor: '#E8E2D6', color: '#1A1818', backgroundColor: '#FDFCFA' }}
                    >
                            ...
                            {drawerItems.map((section: any, index: number) => {
                                return (
                                    <Accordion.Item key={index} value={section.props.title}>
                                        <Accordion.Header>
                                            <Accordion.Trigger className="flex w-full items-center justify-between px-4 py-2 text-xs text-[#6F6863] font-semibold uppercase hover:bg-[#FAF7F2] transition-colors [&[data-state=open]>svg]:rotate-180">
        ...
        drawerItem: ({ name }) => {
            return (
                <div className="flex items-center bg-[#FDFCFA] border-[#E8E2D6] rounded-md flex-col py-5 border px-2 gap-2 text-center">
