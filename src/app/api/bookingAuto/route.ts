...
export async function POST(request: NextRequest) {
    const { paymentIntentID, businessId, policyId, serviceId, client_metadata, timeSlot, 
        elements ,
        stripe} : {paymentIntentID: string, businessId: string, policyId: string, serviceId: string, client_metadata: any, timeSlot: {
            start?: string;
            end?: string;
            appointmentLength: number;
        }, 
            elements: StripeElements | null,
            stripe: Stripe | null} = await request.json();
            ...
            try {
                ...
                if(businessPolicy.rows[0].deposit.enabled && elements){
                    ...
                    const {error} = await stripe?.confirmPayment({
                        ...
                        elements,
                        confirmParams: {
                          return_url: `http://localhost:3000/appointment/${appointment.rows[0].id}/complete`,
