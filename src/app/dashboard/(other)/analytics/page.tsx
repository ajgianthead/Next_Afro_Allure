...
export default async function Page() {
    ...
    if (user) {
        ...
        const { data, error } = await supabase.from('appointments').select("*, business_users(plan_type)").eq('business', business_user.business_id).neq('status', 'PENDING')
            .neq('status', 'DENIED')
            .neq('status', 'PROCESSING').gt('created_at', pastNinetyDays).lt('created_at', endOfTheDayToday)
        if (data?.length && data[0].business_users.plan_type === 'STARTER') {
            ...
            return <div className="w-full  flex h-[500px] justify-center items-center">
                <SubscribeCard customerID={business_user.stripe_customer_id!} businessID={business_user.business_id} hadTrial={business_user.had_trial} />
            </div>
        }
