...
import { createSubscriptionForExistingCustomer } from "app/for-businesses/actions";
...
export default async function Page() {
    ...
    const { data: completedAppointments }: any = await supabase.from('appointments').select().eq('business', business_user.business_id).eq('status', 'COMPLETED').gte('created_at', DateTime.now().startOf('month').toISO()).lte('created_at', endOfTheDayToday)
    const isGrowth: boolean = business_user.plan_type === "GROWTH"
    if (isGrowth && data?.length) {
        ...
        return <AnalyticsClient isGrowth={isGrowth} webTrafficData={webTrafficSourceData} serviceCountMap={countMap} appointmentCompletionRate={completedRate} appointments={data!} business_user={business_user} pageViews={pageViewData} monthlyBookings={thisMonthsBookings} defaultMonthlyAppointmentsData={result} />;
    }
    return <AnalyticsClient isGrowth={isGrowth} appointmentCompletionRate={0} appointments={[]} business_user={business_user} pageViews={[]} monthlyBookings={0} defaultMonthlyAppointmentsData={[]} />;
}
