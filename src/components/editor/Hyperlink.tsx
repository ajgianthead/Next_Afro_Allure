...
export const Hyperlink: UserComponent = ({ color = '#000000', text = 'Insert text...', padding = 'auto', margin = 'auto', fontFamily = 'Open Sans', textAlign = 'center', fontSize = 16, fontWeight = 'normal', fontStyle = 'none', textDecorationLine = 'none', lineHeight = 1.5, letterSpacing = 0, src }: any) => {
    ...
    return (
        <div ref={(ref: any) => connect(drag(ref))} className={`max-w-max max-h-min ${selectedNode.values().toArray()[0] === id ? "border-solid border-blue-800" : ""} ${isHovering && selectedNode.values().toArray()[0] !== id ? `border-dashed border-blue-800 border-2` : ''} ${isHovering && selectedNode.values().toArray()[0] === id ? `border-solid border-blue-800 border-2` : ''} ${!isHovering && selectedNode.values().toArray()[0] === id ? `border-solid border-blue-800 border-2` : ''} `}>
            ...
            <a className="w-full h-full" href={src} style={{
                color,
                padding,
                margin,
                fontSize,
...
const HyperlinkSettings = () => {
    ...
    return (
        <div>
            <div>
                <Caption className='font-bold text-slate-300  mb-2'>Properties</Caption>
                <div className="flex flex-col gap-2">
                    <div className='w-full justify-between items-center flex'>
                        <Caption>Text</Caption>
                        <Input className='w-2/3 text-xs' value={props.text} onChange={(e) => setProp((props: any) => props.text = e.target.value)} />

                    </div>
                    <div className='w-full justify-between items-center flex'>
                        <Caption>URL</Caption>
                        <Input className='w-2/3 text-xs' value={props.src} onChange={(e) => setProp((props: any) => props.src = e.target.value)} />

                    </div>
                </div>

                <Caption className='font-bold text-slate-300  mt-2'>Layout</Caption>
                            ...
                            <div className='w-4/6 py-[1px]'>
                                <Popover.Root>
                                    <Popover.Trigger asChild>
                                        ...
                                        }}></div>{props.color}</MUIButton>
                                    </Popover.Trigger>
                                    <Popover.Portal>
                                        <Popover.Content mixed className="max-w-xs">
                                            <SketchPicker color={props.color} onChangeComplete={(color: any, event: any) => {
