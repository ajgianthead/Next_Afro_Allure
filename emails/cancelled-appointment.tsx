...
export default function CancelledAppointment({ socials, serviceName, clientData, businessData, appointmentData }: EmailTemplate) {
    return (
        <Tailwind config={{
            theme: {
                extend: {
            ...
            <Html className="px-5 font-sans">
                <Preview>{`${clientData.firstName} just cancelled their appointment`}</Preview>
                ...
                <Container className="font-sans w-full text-center">
                    <Heading as="h2">‼️Cancelled Appointment Alert‼️</Heading>
                    <Text>{`${clientData.firstName} just cancelled their appointment on ${DateTime.fromISO(appointmentData.start).toFormat('LLLL dd, yyyy')} @ ${DateTime.fromISO(appointmentData.start).toLocaleString(DateTime.TIME_SIMPLE)}`}</Text>
                </Container>
                <Hr />
...
CancelledAppointment.PreviewProps = {
    socials: {
