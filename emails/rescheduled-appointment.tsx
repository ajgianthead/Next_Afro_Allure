...
export default function RescheduledAppointment({ socials, serviceName, clientData, businessData, appointmentData }: EmailTemplate) {
    return (
        <Tailwind config={{
            theme: {
                extend: {
            ...
            <Html className="px-5 font-sans">
                <Preview>{`Your appointment with ${businessData.name} has been successfully rescheduled`}</Preview>
                ...
                <Container className="font-sans w-full text-center">
                    <Heading as="h2">‼️Reschedule Alert‼️</Heading>
                    <Text>{`${clientData.firstName} just rescheduled on ${DateTime.fromISO(appointmentData.start).toFormat('LLLL dd, yyyy')} @ ${DateTime.fromISO(appointmentData.start).toLocaleString(DateTime.TIME_SIMPLE)}`}</Text>
                </Container>
                <Hr />
                ...
                <Container className="font-sans mt-5">
                    <div className="text-center">
                        <Button href={`${process.env.NODE_ENV === 'development' ? process.env.NEXT_PUBLIC_BASE_URL : process.env.NEXT_PUBLIC_PROD_BASE_URL}/appointments`} className="bg-black text-white p-5 font-bold rounded">View Appointment Details</Button>
                    </div>
                </Container>
...
RescheduledAppointment.PreviewProps = {
    socials: {
